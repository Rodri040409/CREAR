...
export default function SavoyeTopExact() {
  ...
  const NAV_TOP = { headerBg: "rgba(0,0,0,0)", logoPad: "px-[2.2rem] py-[1.4rem]", cardVisible: "opacity-100" };
  const NAV_SCROLL = { headerBg: "#ffffff", logoPad: "px-[2.2rem] py-[1.4rem]", cardVisible: "opacity-0" };
  ...
  // helper para crear items válidos para ServiceItem (con href requerido)
  const svc = (slug: string, icon: string, label: string): ServiceItem => ({
    slug,
    icon,
    label,
    href: `/servicios/${slug}`,
  ...
  const services: ServiceItem[] = [
    ...
    svc("obra-publica", "/imagenes/Iconos/icono3.png", "Obra pública"),
    svc("urbanizacion", "/imagenes/Iconos/icono4.png", "Urbanización"),
    svc("electrificacion", "/imagenes/Iconos/icono5.png", "Electrificación"),
    svc("gestion-de-proyectos", "/imagenes/Iconos/icono6.png", "Gestión de proyectos"),
  ];
  ...
  const startTimer = useCallback(() => {
    clearTimer();
    timerRef.current = window.setInterval(() => setIdx((i) => (i + 1) % slides.length), 7000);
  ...
  return (
    <section id="home" className="relative overflow-x-hidden" style={{ fontFamily: "Roboto, system-ui, -apple-system" }}>
      {fontLinks.map((href) => (
      ...
      >
        <div className="mx-auto flex w-full max-w-[114rem] items-center justify-center md:justify-between px-[1.5rem]">
          ...
          >
            ...
            />
            <img src={logoLight} alt="CREAR" className="relative h-[3.6rem] md:h-[2.8rem] select-none" draggable={false} />
          </a>
        </div>
      </motion.header>

      {/* HERO */}
      <div className="relative overflow-hidden" style={{ height: "calc(var(--vh, 1vh) * 100)", minHeight: "100svh" }}>
        {slides.map((s, i) => {
          const active = i === idx;
          const isFirst = i === 0;
